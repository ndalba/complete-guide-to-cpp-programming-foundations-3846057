...
public:
    ...
    //Destructor
    ~Inventory(){
        delete items; //Prevent memory leak by deallocatinh the dynamic vector
    }
...
private:
    std::vector<std::string> *items; //Pointer to a vector if items
    int capacity; //Maximim number of items allowed
    
    
};

int main(){
    //Creat an inventory with capacity of 5 items
    ...
    //Try to add another item when the inventory is full
    ...
    std::cout << "There are " << myInventory.getItemCount() << " items in the inventory." << std::endl;

    //Access item by index
    std::cout << "The item in position 2 is " << myInventory.getItem(2) << std::endl;
